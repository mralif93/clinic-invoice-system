<x-layouts.admin title="Access Roles & RBAC Matrix">
    ...
    <div class="space-y-8">
        ...
        <x-card class="p-4">
            <form method="GET" action="{{ route('admin.roles.index') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-3 text-xs">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex items-center justify-end gap-1.5">
                                        ...
                                        @if(!$role->is_system && $role->users->count() === 0)
                                            <form action="{{ route('admin.roles.destroy', $role->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete the role {{ $role->display_name }}?');" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button 
                                                    type="submit"
