<x-layouts.admin title="Staff User & Role Access Control">
    ...
    <div class="space-y-8">
        ...
        <x-card class="p-4">
            <form method="GET" action="{{ route('admin.users.index') }}" class="grid grid-cols-1 sm:grid-cols-12 gap-3 text-xs">
                <!-- Search Input -->
                                ...
                                <td class="py-4 px-4 sm:px-6 text-right">
                                    <div class="inline-flex items-center gap-1.5 justify-end">
                                        ...
                                        @if($user->id !== auth()->id())
                                            ...
                                            <!-- Delete User -->
                                            <form method="POST" action="{{ route('admin.users.destroy', $user) }}" class="inline" onsubmit="return confirm('Permanently delete staff account for {{ addslashes($user->name) }}? This action cannot be undone.')">
                                                @csrf
                                                @method('DELETE')
                                                <button 
                                                    type="submit" 
    ...
    <x-modal name="create-user" title="Register New Clinic Staff Account" size="lg">
        <form method="POST" action="{{ route('admin.users.store') }}" class="p-6 space-y-4 text-left">
            @csrf
