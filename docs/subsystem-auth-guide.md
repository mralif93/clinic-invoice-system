...
1. Ensure **CentraFlow** is running:
   ...
   ```
2. Navigate to your sub-system's login page:
   - **HRMS**: `http://localhost:8001/login`
   - **Payroll**: `http://localhost:8002/login`
   - **Clinic Invoicing (CIS)**: `http://localhost:8003/login`
...
5. Enter CentraFlow credentials:
   - **Email:** `superadmin@centraflow.local` (or `hr.manager@centraflow.local`)
   - **Password:** `password`
6. CentraFlow prompts for scope authorization (or auto-approves for first-party clients).
7. You are redirected back to the sub-system callback URL (e.g. `http://localhost:8003/auth/callback`) and seamlessly logged in!

---

## 6. Central Single Logout (SLO) & Alert Handling

When a user logs out from a sub-system:
1. Sub-system invalidates local session and tokens (`Auth::logout()`, `$request->session()->invalidate()`).
2. Redirects to CentraFlow central logout endpoint:
   ```php
   ...
   ```
3. CentraFlow clears the centralized single sign-on cookie and redirects back to `redirect_uri`.
4. Sub-system login page renders a standard dismissible emerald notification:
   > *"You have been logged out securely."*

---

## 7. Global Session Management & Token Revocation
